...
export default function Subway(props: props) {
    ...
    if (
        ...
        props.station === "Проспект Ветеранов"
    ) {
        backgorund = "#d10600";
    }

    if (
        props.station === "Парнас" ||
    ...
    let text = <p className={props.v2 ? "--v2" : ""}>{props.station}</p>;

    if (props.text_style) {
        if (props.text_style === "h4")
            text = <h4 className={props.v2 ? "--v2" : ""}>{props.station}</h4>;

        if (props.text_style === "h3")
            text = <h3 className={props.v2 ? "--v2" : ""}>{props.station}</h3>;
    }
