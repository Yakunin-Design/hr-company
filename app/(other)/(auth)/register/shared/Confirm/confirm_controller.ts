...
export default function confirm_controller() {
    ...
    function on_change(event: any) {
        ...
        if (regex.test(value)) {
            ...
            if (name != 'CodeValue4' && elem_name != 'CodeValue0') {
                const elem = document.querySelector(`#${elem_name}`) as HTMLInputElement
                elem.focus();
            }
            else {
                const elem = document.querySelector("#CodeValue4") as HTMLInputElement
                elem.blur();
            }
        }
    ...
    async function registration_api_call(data: any) {
        data.payload.phone = data.payload.phone.replace(/\s/g,'').split("(").join("").split(")").join("")
