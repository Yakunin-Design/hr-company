...
async function get_chats(req: Request, res: Response) {
    ...
    async function parse_chat(chat: any) {
        const another_user_id = chat.user1.toString() === id.toString() ? chat.user2 : chat.user1;
        ...
        if (another_user.Ok === null) {
            another_user = await db.find({ _id: another_user_id }, user_type === 'workers' ? 'employers' : 'workers');
        ...
        const unread_messages = chat.user1.toString() === id.toString() ? chat.unread_count1 : chat.unread_count2;
    ...
    const chats = await db.find_all({ $or: [{ user1: id }, { user2: id }] }, 'chats');
    ...
    const response = await Promise.all(chats.Ok.map(async (chat) => await parse_chat(chat)));
...
async function get_messages(req: Request, res: Response) {
    ...
    if (chat.Ok.user1.toString() != res.locals.user._id.toString() && chat.Ok.user2.toString() != res.locals.user._id.toString()) {
        return res.status(404).send(':(');
    }

    const user = res.locals.user._id.toString() === chat.Ok.user1.toString() ? chat.Ok.user2 : chat.Ok.user1;
    ...
    const msgs = chat.Ok.msgs.map( msg => {
        ...
        return {
            ...msg,
            sender: (user_id === user1 && msg.sender === 1) ? true : (user_id === user2 && msg.sender === 2) ? true : false
        }
    });
    ...
    const user_notify = res.locals.user._id.toString() === chat.Ok.user1.toString() ? { unread_count1: 0 } : { unread_count2: 0 };

    const notify_reset = await db.update({ _id: chat_id }, { $set: { ...user_notify } }, 'chats');

    if (notify_reset.Err) return res.status(400).send('unable to get messages :(');

    res.status(200).send(response);
} 

async function create_chat(my_user_type: string, my_user_id: string, another_user_id: string): Promise<ObjectId | Error> {

    const chat = {
        user1: my_user_type === 'employer' ? new ObjectId(my_user_id) : new ObjectId(another_user_id),
        user2: my_user_type != 'employer' ? new ObjectId(my_user_id) : new ObjectId(another_user_id),
        msgs: [],
        unread_count1: 0,
        unread_count2: 0
    };
...
async function send_message(req: Request, res: Response) {
    ...
    if (another_user_id) {
        chat_id = await create_chat(res.locals.jwt.user_type, res.locals.user._id, another_user_id);
    ...
    if (res.locals.user._id.toString() != chat.Ok!.user1.toString() && res.locals.user._id.toString() != chat.Ok!.user2.toString()) {
    ...
    const new_msg = {
        ...
        sender: res.locals.user._id.toString() === chat.Ok!.user1.toString() ? 1 : 2,
        content: message
    };

    const messages = [...chat.Ok!.msgs, new_msg];

    const unread_count1 = res.locals.user._id.toString() === chat.Ok!.user1.toString() ? chat.Ok!.unread_count1 : chat.Ok!.unread_count1+1;
    const unread_count2 = res.locals.user._id.toString() === chat.Ok!.user1.toString() ? chat.Ok!.unread_count2+1 : chat.Ok!.unread_count2;
    ...
    const db_result = await db.update({_id: new ObjectId(chat_id)}, {$set: {...edits}}, 'chats');
...
async function read_messages(req: Request, res: Response) {
    try{
        ...
        const unread_messages = my_id === chat.Ok!.user1.toString() ? {unread_count1: 0} : {unread_count2: 0}
