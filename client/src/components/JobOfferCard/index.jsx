...
function JobOfferCard(props) {
    ...
    if (props.data.schedule) {
        for (let i = 0; i < props.data.schedule.weekdays; i++) {
            schedule_blocks.push(
                <div className="schedule-block --weekdays"></div>
            );
        }
        for (let i = 0; i < props.data.schedule.weekends; i++) {
            schedule_blocks.push(
                <div className="schedule-block --weekends"></div>
            );
        }
    }
        ...
        <>
            {full_job_offer && (
                <DisplayJobOffer
                    id={props.id}
                    key={props.id}
                    handle_click={toggle_full_job_offer}
                    worker
                />
            )}
            <div className="card JobOfferCard" onClick={toggle_full_job_offer}>
                <div className="JobOfferCard__place">
                    <div className="JobOfferCard__company">
                        {props.data.logo ? (
                            <img
                                src={props.data.logo}
                                className="JobOfferCard__company-logo"
                            />
                        ) : (
                ...
                <hr />
                <h3 className="JobOfferCard__price">
                    {props.data.salary.amount}₽ -{' '}
                    {props.data.salary.period === 'month'
                        ? 'месяц'
                        : props.data.salary.period === 'day'
                        ? 'смена'
                        : 'час'}
                </h3>
            </div>
