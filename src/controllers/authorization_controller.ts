...
async function login(req: Request, res: Response): Promise<void> {
    try {
        ...
        const result_employer = await db.find(
        ...
        if (result_employer.Ok != null) {
            const passwords_match = await bcryptjs.compare(
                password,
                result_employer.Ok.password
            );
            ...
            const result_worker = await db.find(
                { $or: [{ email: login }, { phone: login }] },
                "workers"
            );
...
async function signup(req: Request, res: Response): Promise<void> {
    try {
        ...
        if (
            user_type == "employer" &&
            (payload.citizenship || payload.birthday || payload.specialty)
        ) {
            res.status(400).send("Debil");
            return;
        }
        ...
        // Validating input
        const validate: Result<IEmployer | IWorker> =
            user_type === "worker"
